    ...
    <div className={`message-fade-in group w-full ${isUser ? 'bg-transparent' : 'bg-transparent'} animate-slide-in-up`}>
      <div className="max-w-4xl mx-auto px-2 sm:px-4 py-2 sm:py-4">
        <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
          <div className={`
            max-w-[95%] sm:max-w-3xl p-3 sm:p-4 rounded-2xl sm:rounded-3xl shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-1
            ${isUser 
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                  {isUser && (
                    <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center overflow-hidden border-2 border-blue-300 dark:border-blue-600 flex-shrink-0 bg-blue-100 dark:bg-blue-800">
                      <img 
                  ...
                  {!isUser && (
                    <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center bg-gray-600 dark:bg-gray-500 text-white font-medium text-xs flex-shrink-0">
                      <svg className="w-2 h-2 sm:w-3 sm:h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                      </svg>
                    </div>
                  )}
                  <span className={`font-medium text-xs sm:text-sm ${isUser ? 'text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300'}`}>
                ...
                <div className={`prose prose-sm max-w-none ${isUser ? 'prose-blue dark:prose-invert' : 'prose-gray dark:prose-invert'}`}>
                  <ReactMarkdown
                    components={{
                      ...
                      blockquote: ({ children }) => (
                        <blockquote className="border-l-4 border-gpt-blue-500 pl-4 py-2 my-4 bg-gray-50 dark:bg-gray-700/50 rounded-r-lg italic text-gray-600 dark:text-gray-400">
                          {children}
                        </blockquote>
                      ),
                      strong: ({ children }) => (
              ...
              {!isUser && !message.isTyping && !isEditing && (
                <div className="flex items-center space-x-1 mt-2 mb-1 sm:mb-2">
                  <button
                    onClick={() => handleReaction('like')}
                    className={`flex items-center space-x-1 px-1.5 sm:px-2 py-1 rounded-full text-xs transition-all hover:bg-gray-200 dark:hover:bg-gray-600 ${
              ...
              {/* Enhanced Actions */}
              <div className="flex items-center justify-between mt-2 sm:mt-3 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="flex items-center space-x-1 sm:space-x-2">
                  ...
                  {isUser && !message.isTyping && (
                    <>
                      <button
                        ...
                        className="flex items-center space-x-1 text-xs text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-200 px-1 sm:px-2 py-1 rounded-full hover:bg-red-100 dark:hover:bg-red-900 transition-colors"
                      >
                  ...
                  {!isUser && !message.isTyping && onRegenerate && (
                    <button
                      ...
                      className="flex items-center space-x-1 text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 px-1 sm:px-2 py-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                    >
                ...
                <button
                  ...
                  className="flex items-center space-x-1 text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 px-2 py-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                >
